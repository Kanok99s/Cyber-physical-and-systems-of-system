...
/*  
    ...
    It then identifies contours with an area greater than a threshold value, draws them on a separate image, and displays the result in
    a window. 
    ...
    Creates a new cv::Mat object named cannyImg to store the result of applying Canny edge detection to the input image using the 
    provided threshold values.
    Uses the cv::findContours function to find contours in the cannyImg image.
    Checks if the area of the current contour (cv::contourArea(contours[i])) is greater than 60. If it is, it means that a potential 
    cone has been found.
